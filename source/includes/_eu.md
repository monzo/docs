...
TPPs must use a self-signed (or not signed) SSA. This is where the `alg` claim is set to `none`. The SSA's `orgID` must match the QWAC's organisation identifier.
