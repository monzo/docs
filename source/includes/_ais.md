...
We support an optional query param `includePots` when set to `true` an additional `Information` balance type will be returned. This balance is the aggregated account balance made up of the account balance plus the balance of each pot owned by the account.
